...
==================================
Owner: Raoul Kahn
Last Updated: March 22, 2026 — 1:49 PM
...
FOR RAOUL (manual tasks):
  [ ] Fix headshot size on Firefox — test and tell CC what size to adjust
  [ ] Add more images to /spin case study page (App Clips, Apple Pay, autonomous vehicle, etc.)
  [ ] Add more images to /autotune case study page
  [ ] Find and add a better Airbnb logo (current one works but could be cleaner)
  [ ] Download real tool logos for future pill badges (Claude, n8n, Figma, etc.) — list of brand pages saved in project chat
  [ ] Add www.raoulkahn.com as domain in Vercel settings (SSL issue with www)
  [ ] Side-by-side compare case study pages with Wix originals to verify CC didn't rewrite copy
  [ ] Daily To Do page has 2 PDFs wired up — verify they download correctly
  [ ] Decide on tab naming: "AI Projects" vs "Professional" — consider renaming
  [ ] Chrome caches old SSL error — test in incognito, may need to clear cache

FOR CC (prompt needed):
  [ ] Fix headshot photo size on Firefox (too small)
  [ ] Add light border/frame treatment to dark screenshots on portfolio cards (Agentic Content Reviewer, AI Run Partner)
  [ ] Match case study page design to Figma Blue Grey template (dark hero banner, spacing, typography)
  [ ] Wire up any new images Raoul adds to /spin and /autotune folders
  [ ] Copy audit — CC should print all text from each case study page so Raoul can verify against originals
  [ ] Add "View Case Study →" link to any cards still showing "Coming Soon" once pages are ready
  [ ] Mention Lenny's Podcast on Product Expert Insights card (carousel thumbnail is Lenny-branded but card text doesn't reference him)
